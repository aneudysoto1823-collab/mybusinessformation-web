...
Problemas con el dominio `opabiz.com`, registros DNS, certificados SSL, propagación, redirects.

---

### 1. opabiz.com no carga (sitio principal caído)
**Status:** 🔴 Crítico
**Síntoma visible:** Al entrar a `https://opabiz.com` el navegador muestra "This site can't be reached" o "DNS_PROBE_FINISHED_NXDOMAIN". El sitio en `opabiz.com` SÍ funciona, pero el dominio custom no.
**Solución posible:** Verificar primero https://www.vercel-status.com — descartar problema de Vercel. Ir a https://dnschecker.org → buscar `opabiz.com` con tipo "A" o "CNAME" desde múltiples regiones. Si NO resuelve a las IPs de Vercel (76.76.21.x), problema de DNS. Ir al provider del dominio (Cloudflare/Namecheap/GoDaddy) → DNS settings → verificar registros: debe haber A o CNAME apuntando a Vercel. Comparar con instrucciones en Vercel Dashboard → Settings → Domains → click sobre el dominio → ver registros esperados. Corregir en provider, esperar propagación 5-30 min.
...
**Solución posible:** Vercel maneja SSL automáticamente con Let's Encrypt — los certificados se renuevan solos cada 90 días. Si expiró, hay un problema. Ir a Vercel Dashboard → Settings → Domains → click sobre `opabiz.com` → ver estado SSL. Si dice "Error" o "Pending verification", revisar que los DNS records estén correctos (Vercel necesita validar ownership). Si DNS bien pero SSL no se renovó, click "Refresh" en Vercel para reintentar. Como hotfix temporal: redirigir tráfico al `opabiz.com` que tiene SSL de Vercel siempre válido.

---

### 3. www.opabiz.com no funciona pero el sin www sí (o viceversa)
**Status:** 🟡 Medio
**Síntoma visible:** `opabiz.com` carga correctamente pero `www.opabiz.com` da error o viceversa. Inconsistencia de qué versión funciona.
**Solución posible:** Ir a Vercel Dashboard → Settings → Domains → asegurar que AMBOS están listados: `opabiz.com` (apex) y `www.opabiz.com`. Configurar redirect automático en Vercel: una de las dos debe ser canónica (recomendado: sin www → primary), la otra redirige. Ir al provider de DNS y verificar que `www` tiene registro CNAME apuntando a Vercel (ej: `cname.vercel-dns.com`). Esperar propagación.

---

### 4. Email a `support@opabiz.com` rebota o no llega
**Status:** 🟡 Medio
**Síntoma visible:** Cliente envía email a `support@opabiz.com` y rebota con "Mailbox not found" o "Domain has no mail exchanger". O simplemente no llega a tu inbox.
...
**Solución posible:** Casi siempre es problema del usuario, no del sitio: caché de DNS local de su computadora o ISP con datos viejos. Pedir al usuario: (1) Probar desde otro navegador. (2) Probar desde data móvil (no WiFi). (3) Limpiar DNS cache: `ipconfig /flushdns` (Windows) o `sudo dscacheutil -flushcache` (Mac). (4) Probar desde https://www.isitup.org/opabiz.com — si dice "site is up", confirma que solo es ese usuario. Como segundo paso: verificar geográfico — algunos ISPs en Latam tienen DNS muy desactualizados.

---

### 9. Subdominio (ej: status.opabiz.com) no funciona
**Status:** 🟢 Bajo
**Síntoma visible:** Configuraste subdominio `status.opabiz.com` para BetterStack o `blog.opabiz.com` para futuro blog, no carga. NXDOMAIN.
**Solución posible:** Ir al DNS provider → agregar CNAME para el subdominio apuntando al destino. Ejemplo BetterStack: CNAME `status.opabiz.com` → `statuspage.betterstack.com`. Verificar en dnschecker.org que propagó. En el servicio destino (BetterStack) verificar que aceptó el subdominio. Esperar emisión SSL (Let's Encrypt automático en BetterStack — toma 5-30 min después de DNS propagado).
