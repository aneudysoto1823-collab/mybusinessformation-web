...
// ─── Email template — VIP Compliance Package (2026-09-11) ────────────────────
// Mismo membrete/recuadro/disclosure/footer que buildEmail (arriba) — la
// diferencia es el cuerpo y la oferta: en vez de listar 3 servicios sueltos,
// ofrece el combo Agente Registrado + Declaración Anual (mismo precio que el
// bundle-compliance-ra-ar del checkout) con el link apuntando al landing
// dedicado /vip. Inspirado en un email real de un competidor (US Filing
// Services) que el founder recibió — el 3er punto de valor ("we monitor and
// file every year") es una promesa que ya cumplimos vía suscripciones reales
// de Stripe, no un servicio de soporte aparte sin respaldo.
function buildVipEmail(company: {
  id: string
  document_id: string
  company_name: string
  company_type: string
  ...
  const subject = isEs
    ? `OpaBiz: Su Declaración Anual de Florida se acerca — ${company.company_name}`
    : `OpaBiz: Your Florida Annual Report is coming up — ${company.company_name}`

  const title = isEs
    ? 'RECORDATORIO DE CUMPLIMIENTO ANUAL DE FLORIDA'
    : 'FLORIDA ANNUAL COMPLIANCE REMINDER'

  const greeting = isEs
    ? `${company.company_name} sigue activa en los registros del Estado de Florida.`
    : `${company.company_name} is still active on Florida's state records.`

  const p1 = isEs
    ? 'Toda LLC y Corporación de Florida debe presentar una Declaración Anual cada año ante la División de Corporaciones y mantener un Agente Registrado con dirección física en el estado. No presentar a tiempo resulta en una multa de $400 y puede llevar a la disolución administrativa de la empresa.'
    : 'Every Florida LLC and Corporation must file an Annual Report each year with the Division of Corporations and maintain a Registered Agent with a physical Florida address. Missing the deadline results in a $400 late fee and can lead to administrative dissolution.'

  const p2 = isEs
    ? 'Para simplificar esto, ofrecemos un Paquete VIP de Cumplimiento que combina ambos requisitos en una sola suscripción anual — nosotros monitoreamos las fechas y presentamos su Declaración Anual, sin que usted tenga que acordarse.'
    : 'To make this simple, we offer a VIP Compliance Package that bundles both requirements into a single annual subscription — we monitor the deadlines and file your Annual Report for you, so you never have to remember.'

  const offerHtml = `
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="border:1.5px solid #2563EB;border-radius:12px;overflow:hidden">
      <tr><td style="background:#2563EB;padding:11px 20px;text-align:center"><span style="color:#fff;font-weight:700;font-size:12px;letter-spacing:.5px;text-transform:uppercase">${isEs ? 'Paquete VIP de Cumplimiento' : 'VIP Compliance Package'}</span></td></tr>
      <tr>
        <td style="background:#fff;padding:20px 24px 6px;text-align:center">
          <div style="color:#1C2E44;font-size:15px;font-weight:800;margin-bottom:10px">${isEs ? 'Agente Registrado + Declaración Anual' : 'Registered Agent + Annual Report'}</div>
          <div><span style="font-family:Georgia,serif;font-size:30px;font-weight:800;color:#1C2E44">$178</span><span style="font-size:13px;color:#64748b;font-weight:600">${isEs ? '/año' : '/year'}</span></div>
          <div style="color:#94A3B8;font-size:11px;margin-top:2px">${isEs ? '+ $139 de tarifa estatal de Florida' : '+ $139 Florida state filing fee'}</div>
        </td>
      </tr>
      <tr>
        <td style="background:#fff;padding:14px 24px 22px">
          <table cellpadding="0" cellspacing="0" border="0" width="100%" style="border-top:1px solid #F1F5F9;margin-top:6px">
            <tr><td style="padding:12px 0 0;color:#475569;font-size:12.5px;line-height:1.6"><strong style="color:#1C2E44">${isEs ? 'Agente Registrado' : 'Registered Agent'}</strong> — ${isEs ? 'dirección oficial en FL para correspondencia legal y estatal, renovada cada año.' : 'official FL address for state &amp; legal correspondence, renewed every year.'}</td></tr>
            <tr><td style="padding:10px 0 0;color:#475569;font-size:12.5px;line-height:1.6"><strong style="color:#1C2E44">${isEs ? 'Declaración Anual' : 'Annual Report Filing'}</strong> — ${isEs ? 'requerida cada año ante el estado, renovada automáticamente.' : 'required every year with the state, renewed automatically.'}</td></tr>
            <tr><td style="padding:10px 0 0;color:#475569;font-size:12.5px;line-height:1.6">${isEs ? 'Nosotros monitoreamos y presentamos su Declaración Anual cada año, dentro del plazo correspondiente.' : 'We monitor your compliance and file your Annual Report every year, right on time.'}</td></tr>
          </table>
        </td>
      </tr>
    </table>`
      ...
      <table cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;width:100%">
        ...
        <tr>
          <td style="background:#fff;padding:20px 36px 6px">
            <p style="color:#1C2E44;font-size:15px;font-weight:700;margin:0 0 16px">${greeting}</p>
            <p style="color:#475569;font-size:13.5px;line-height:1.7;margin:0 0 14px">${p1}</p>
            <p style="color:#475569;font-size:13.5px;line-height:1.7;margin:0">${p2}</p>
          </td>
        </tr>

        <!-- Oferta VIP -->
        <tr>
          <td style="background:#fff;padding:18px 36px 6px">
            ${offerHtml}
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#fff;padding:24px 36px 6px;text-align:center">
            <a href="${trackUrl}" style="display:inline-block;background:#2563EB;color:#fff;text-decoration:none;padding:15px 44px;border-radius:9px;font-weight:700;font-size:15px">${isEs ? 'Obtener Paquete VIP' : 'Get VIP Compliance Package'} &#8594;</a>
            <div style="color:#64748b;font-size:12px;margin-top:8px">${isEs ? '¿Preguntas? Escríbanos por WhatsApp al ' : 'Questions? WhatsApp us at '}<a href="https://wa.me/13528377755" style="color:#2563EB;text-decoration:none;font-weight:600">+1 (352) 837-7755</a></div>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#fff;padding:18px 36px 26px">
            <div style="border-top:1px solid #eef2f7;padding-top:16px">
              <div style="color:#1C2E44;font-size:11px;font-weight:800;letter-spacing:.5px;text-transform:uppercase;margin-bottom:6px">${isEs ? 'Aviso Importante' : 'Important Disclosure'}</div>
              <p style="color:#94A3B8;font-size:10.5px;line-height:1.7;margin:0">${isEs
                ? 'Florida Business Formation Center es un servicio profesional de preparación y presentación de documentos. No somos un bufete de abogados y no brindamos asesoría legal, fiscal ni financiera. Nuestros servicios no constituyen el ejercicio de la abogacía ni crean una relación abogado-cliente. Todas las presentaciones están sujetas a la aprobación de la División de Corporaciones de Florida. Florida Business Formation Center no está afiliado, respaldado ni aprobado por ninguna agencia gubernamental federal, estatal o local. Este aviso no es una factura ni una solicitud de pago. La suscripción es opcional y puede cancelarse en cualquier momento.'
                : 'Florida Business Formation Center is a professional document preparation and filing service. We are not a law firm and do not provide legal, tax, or financial advice. Our services do not constitute the practice of law and do not create an attorney-client relationship. All filings are subject to approval by the Florida Division of Corporations. Florida Business Formation Center is not affiliated with, endorsed by, or approved by any federal, state, or local government agency. This notice is not a bill, invoice, or demand for payment. The subscription is optional and may be cancelled at any time.'}</p>
            </div>
          </td>
        </tr>
...
export async function POST(req: NextRequest) {
  ...
  try {
    // campaign: 'compliance' (default, carta de cumplimiento con 3 servicios
    // sueltos) | 'vip' (Paquete VIP — Agente Registrado + Declaración Anual,
    // ver buildVipEmail arriba y el landing dedicado /vip).
    const { company_ids, lang = 'en', campaign = 'compliance' } = await req.json()

    if (!Array.isArray(company_ids) || company_ids.length === 0) {
      return NextResponse.json({ error: 'company_ids array is required' }, { status: 400 })
    }
    if (campaign !== 'compliance' && campaign !== 'vip') {
      return NextResponse.json({ error: 'campaign must be "compliance" or "vip"' }, { status: 400 })
    }
    ...
    for (const company of companies) {
      ...
      try {
        ...
        // dest=vip hace que track-scan redirija a /vip en vez de al home.
        const trackUrl = `${BASE_URL}/api/campaigns/track-scan?doc=${encodeURIComponent(company.document_id)}&cid=${company.id}`
          + (campaign === 'vip' ? '&dest=vip' : '')

        // Build email
        const { subject, html: baseHtml } = campaign === 'vip'
          ? buildVipEmail(company, trackUrl, lang as 'en' | 'es')
          : buildEmail(company, trackUrl, lang as 'en' | 'es')
