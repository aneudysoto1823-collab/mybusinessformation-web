...
- **Pie de página repetido "opabiz.com" en cada hoja (2026-07-31):** `.pdf-footer` con `position:fixed; bottom:3mm` — Chrome repite los elementos `position:fixed` en cada página al imprimir (a diferencia de `position:absolute`, que solo aparece una vez), truco que evita depender de CDP/`printToPDF` con `footerTemplate`. **No aparece en la portada** (queda oculto detrás de la lógica de paginación de esa página especial de una sola hoja) — resultado visualmente correcto de todas formas: es convención estándar que la tapa no lleve el pie de página repetido, ya tiene su propio wordmark "OpaBiz" grande abajo. ⚠️ Si se vuelve a tocar el valor de `bottom`, probar primero en una página densa (texto que llega hasta el borde inferior) — con `bottom:8mm` el pie se superponía con la última línea de texto en páginas así; `3mm` lo resuelve con margen de sobra.
- **Botones CTA con fondo blanco + borde azul (2026-07-31, antes fondo azul sólido):** selector `a[href^="https://opabiz.com"]` (antes solo `opabiz.com/servicios` y `opabiz.com/booking` — se amplió para que `/contact` y `/guia-gratis` también salgan como botón). Cambio de estilo para seguir el mismo concepto ya usado en el sitio (ver botón de `/guia-gratis`, fondo blanco/borde azul en vez de relleno sólido).
...
2. **Deep-link `/servicios?open=<id>`** — antes no existía forma de abrir un servicio específico por URL (todo requería clic manual en la tarjeta). `/servicios?open=dba` (u otro id del catálogo) hace scroll a esa tarjeta y la resalta. Usado por los botones CTA de las Guías I y II.
   - ⚠️ **Comportamiento cambiado 2026-07-31:** originalmente `?open=` llamaba a `openServiceForm(open)` y abría el formulario/checkout directo, saltándose la página de servicios — el founder prefirió que el cliente primero vea el catálogo completo. Ahora llama a `highlightCard(open)` (función que ya existía para el flujo `/servicios#<id>` pero nunca tenía CSS — se le agregó `.svc-acc-item.highlighted` con anillo azul + animación de pulso). Cambio vive 100% en `backend/app/servicios/page.tsx`, no requirió tocar los PDFs de las guías (la URL `?open=` no cambió, solo qué hace la página al recibirla).
...
**Seguimiento de leads — `/admin/guias`:** panel admin nuevo que lista los leads de `guide_sends` con `source:'landing'` (los de campaña/orden ya se administran en sus propios paneles — `/admin/campaigns` y `/admin/orders`). Muestra nombre, email, fecha, y una nota libre editable por lead (mismo patrón de botón 📝 + modal que ya usa `/admin/campaigns` para `prospective_companies.note`). API: `GET/PATCH /api/admin/guide-leads`. Arriba de la tabla hay una tarjeta con botones 👁 Ver / 📄 Descargar para ambas guías (`/guias/*.pdf`), para no tener que ir a buscar el archivo en el repo.

**3 puntos de disparo, todos vía `lib/guides.ts`:**
1. **Landing pública `/guia-gratis`** (nueva, campos **nombre + email**, bilingüe EN/ES) — pensada para links en bio de Instagram/TikTok/Facebook (un link normal funciona igual en cualquier red social). Envía `POST /api/guides/request` → si el email nunca recibió la Guía I, la manda (adjunta + link, saludo personalizado con el nombre) y registra `source:'landing'` (con `name`); si ya la tiene, responde éxito sin reenviar ("revisá tu correo, ya te la enviamos antes"). Rate limit `checkGuideRequestRateLimit` (5/h/IP, `lib/rate-limit.ts`). Botón de envío: fondo blanco + borde azul, sin flecha (mismo concepto que el resto del sitio).
...
- `/order/complete`: nueva caja avisando que junto al email de confirmación también se envía de regalo la Guía gratuita de formación, con resumen breve de qué cubre (Agente Registrado, EIN, Operating Agreement, ITIN, etc.). Sin emoji (ver fix 2026-08-07 abajo — se sacó el 🎁 que tenía al lanzarse).

---

## Sesión 2026-08-07 — copy y polish visual del form

- **Texto de dirección virtual (paso 2, `biz-virtual-confirm-text`)**: cambiado de "Once your order is confirmed, we will assign your Florida virtual address..." a "Once your dedicated Florida virtual address is assigned, we will deliver it to your email." — ya no ata la promesa a la confirmación de la orden (que ocurre al pagar) sino al momento real en que se asigna la dirección, que puede tardar más. Aplicado en el HTML default (inglés) y en el objeto de traducciones EN/ES.
- **Caja de regalo de la Guía en `/order/complete`**: se sacó el emoji 🎁 — founder no quiere emojis en el sitio, no se ve profesional para un servicio de trámites legales. Queda solo el texto ("Gift included: ...").
- **Íconos del paso 6 "Boost Your Formation" (`page.tsx`)**: los 13 addons (EIN, Operating Agreement, ITIN, BTR, Sales Tax, Certified Copy, Annual Report, DBA, Banking Resolution, Exclusive Guide, Good Standing, S-Corp, Business License) pasaron de emoji plano a íconos SVG azules en caja redondeada (`background:var(--blue-light)`, `color:var(--blue)`, 40x40px) — mismo componente visual que ya usa `/servicios` (`.svc-acc-icon`). Se reutilizó el mismo mapeo ícono↔servicio que tiene `servicios/page.tsx` (`svgIcons`) para que ambas páginas sean consistentes (ej. Operating Agreement y Certified Copy comparten el ícono `file-text`, EIN usa `hash`, ITIN usa `globe`, etc.). CSS: `.fm-addon-icon` pasó de `font-size:1.3rem` a la caja con `svg{width:19px;height:19px}`.
- Decisión explícita: **NO tocar el autollenado del "Use my business address for mailing"** (paso 3, RA & Mailing Address) — se evaluó cambiarlo a fields editables pre-llenados en vez del preview de solo texto, pero el founder lo descartó en la misma sesión ("olvida lo del business address... ya está así si la gente elige virtual address, si la gente pone su propia dirección"). Queda como estaba: preview read-only con `fmBusinessAddrText()`/mensaje de dirección virtual pendiente.
